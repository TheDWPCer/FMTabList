...
public abstract class AncientAltarCrafter extends SlimefunItem {
	...
	public AncientAltarCrafter(Category category, ItemStack item, String name, RecipeType recipeType, ItemStack[] recipe) {
		super(category, item, name, recipeType, recipe);
		
		new BlockMenuPreset(name, "&6Ancient Altar Crafter") {
	...
	public void register(boolean slimefun) {
		addItemHandler(new BlockTicker() {
			...
			public void tick(Block b, SlimefunItem sf, Config data) {
				AncientAltarCrafter.this.tick(b);
			}
		...
		if (catalyst != null && !catalyst.getType().equals(Material.AIR)) {
			...
			if (result != null && !result.getType().equals(Material.AIR)) {
				if (this.fits(b, new ItemStack[]{result})) {
					...
					for (int j = 0; j < 9; j++) {
						if (menu.getItemInSlot(this.getInputSlots()[j]) != null)
							menu.replaceExistingItem(this.getInputSlots()[j], InvUtils.decreaseItem(menu.getItemInSlot(this.getInputSlots()[j]), 1));
					}
